...
public class ScreenBlockEntityRenderer implements BlockEntityRenderer<ScreenBlockEntity> {
	...
	public void render(ScreenBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		if (be.getScreens() == null || be.getScreens().isEmpty()) return;

		for (ScreenData screen : be.getScreens()) {
			RinkuBrowser browser = ScreenBrowserManager.getBrowser(be.getBlockPos(), screen.side());
			...
			renderTexturedQuad(screen, be.getBlockPos(), texId, poseStack, bufferSource);
		}
	}
	...
	private void renderTexturedQuad(ScreenData screen, BlockPos pos, ResourceLocation texId, PoseStack poseStack, MultiBufferSource bufferSource) {
		...
		VertexConsumer consumer = bufferSource.getBuffer(RenderType.entityCutoutNoCull(texId));
		Matrix4f m = poseStack.last().pose();
		consumer.vertex(m, sx, sy, sz).color(255, 255, 255, 255).uv(0, 1).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LIGHT_FULLBRIGHT).normal(nx, ny, nz).endVertex();
		consumer.vertex(m, sx + rx, sy + ry, sz + rz).color(255, 255, 255, 255).uv(1, 1).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LIGHT_FULLBRIGHT).normal(nx, ny, nz).endVertex();
		consumer.vertex(m, sx + rx + ux, sy + ry + uy, sz + rz + uz).color(255, 255, 255, 255).uv(1, 0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LIGHT_FULLBRIGHT).normal(nx, ny, nz).endVertex();
		consumer.vertex(m, sx + ux, sy + uy, sz + uz).color(255, 255, 255, 255).uv(0, 0).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(LIGHT_FULLBRIGHT).normal(nx, ny, nz).endVertex();
	}
